...
public class RoundProgressBar extends View {
  ...
  public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    roundProgressColor =
        mTypedArray.getColor(R.styleable.RoundProgressBar_roundProgressColor, Color.RED);
    textColor = mTypedArray.getColor(R.styleable.RoundProgressBar_textColor, Color.RED);
    textSize = mTypedArray.getDimensionPixelSize(R.styleable.RoundProgressBar_RPBtextSize,
        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15,
            getResources().getDisplayMetrics()));
    roundWidth = mTypedArray.getDimensionPixelSize(R.styleable.RoundProgressBar_roundWidth, 20);
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    if (textIsDisplayable && style == STROKE) {
      canvas.drawText(text, centre - textWidth / 2, centre + paint.getTextSize() / 2,
          paint); //画出进度百分比
    }
    ...
    switch (style) {
      ...
      case FILL: {
        ...
        if (progress != 0) canvas.drawArc(oval, 270, 360 * progress / max, true, paint);  //根据进度画圆弧
        break;
      }
    }
  }
